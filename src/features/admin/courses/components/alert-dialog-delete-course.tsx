...
export function AlertDialogDeleteCourse({ id, nombreCurso, handleEliminar }: Props) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!response.ok) {
        setFormStatus({
          status: "error",
          message: response.message || "Error desconocido al eliminar el curso",
        });
        return;
      }
      ...
      setFormStatus({
        status: "error",
        message: "Error desconocido al eliminar el curso",
      });
    }
  }
    ...
    <AlertDialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <AlertDialogContent size="sm">
        <AlertDialogHeader>
          ...
          <AlertDialogTitle>{`Eliminar el curso "${nombreCurso}"`}</AlertDialogTitle>
          <AlertDialogDescription>
            ¿Estás seguro de querer eliminar este curso? Esta acción no se
